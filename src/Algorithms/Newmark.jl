...
function Newmark(; Δt::T, α::A, δ::D, check=true) where {T,A,D}
    ...
    if check
        ...
        μ = (1 / 2 + δ)^2 / 4
        α ≥ μ ||
            throw(ArgumentError("expected the parameter α to be at least $μ, but it is $α"))
    end
...
function _solve_statistics(alg::Newmark{N},
                           ...
                           sampler,
                           idx,
                           vecsamples::Vector{Int}) where {N,VT,ST,XT<:Tuple{VT,VT}}
    sys = system(ivp)
    ...
    for k in 1:MAXSAMPLES
        ...
        if k == vecsamples[q]
            hist[:($k)] = Dict(:U_min => copy(U_min),
                               :U_max => copy(U_max),
                               :Udot_min => copy(U′_min),
                               :Udot_max => copy(U′_max))
